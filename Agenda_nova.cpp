...
{
	printf("Cadastrado o registro numero:%d\n",tamarq(arq)+1);
    registro reg;
    ...
    printf("Regitro gravado com sucesso!\n");
    system("pause");
}
...
void relatorio(FILE *arq)
{
	...
	fprintf(arqtxt," Codigo              Nome             Telefone              e-mail\n");
	fprintf(arqtxt,"===========================================================================\n");
	int nr;
	registro reg;
	for(nr=0;nr<tamarq(arq);nr++)
	{
		...
		fread(&reg,sizeof(registro),1,arq);
		fprintf(arqtxt,"%-20s %-17s %-17s %-s\n",reg.Nm,reg.nome,reg.telefone,reg.email);
	}
	fprintf(arqtxt,"===========================================================================\n");
	fclose(arqtxt);
